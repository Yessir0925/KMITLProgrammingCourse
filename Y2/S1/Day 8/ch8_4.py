class AVLTree:
    ...
    def delete(root, data):
        if root is None:
            return None
        if data < root.data:
            root.left = AVLTree.delete(root.left, data)
        elif data > root.data:
            root.right = AVLTree.delete(root.right, data)
        else:
            ...
            root.right = AVLTree.delete(root.right, successor.data)
        root.setHeight()
        if root.balanceValue() < -1:
            if root.left.balanceValue() > 0:
                root.left = AVLTree.rotateRightChild(root.left)
    ...
    def display(self):
        level = [self.root]
        width = 2 ** (self.root.height + 3)
        while level:
            print(''.join(str(node.data).center(width) for node in level))
...
print(' *** AVL Tree ***')
tree = AVLTree()
for value in input('Enter numbers to insert: ').split():
    tree.add(value)
while tree.root:
    tree.display()
    print('-' * 30)
    tree.root = tree.delete(tree.root, tree.root.data)
